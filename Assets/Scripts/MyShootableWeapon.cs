...
using Opsive.UltimateCharacterController.Networking.Game;
    ...
    /// <summary>
    /// 任何可以射击的武器。这包括手枪、火箭发射器、弓箭等。
    /// </summary>
    public class MyShootableWeapon : UsableItem, IReloadableItem, IDamageOriginator
    {
        /// <summary>
        ...
        {
            SemiAuto,   // 半自动，//发射不连续的射击，直到玩家再次发射时才继续发射。
            FullAuto,   // 全自动，//继续射击，直到弹药用完或玩家停止射击。
            Burst       // 突发//保持发射直到突发率为零。
        }
                ...
                if (m_SharedConsumableItemIdentifier && (itemSetManager = m_Inventory.GetComponent<ItemSetManagerBase>()) != null) {
                    for (int i = 0; i < m_Inventory.SlotCount; ++i) {
                        ...
                        for (int j = 0; j < itemActions.Length; ++j) {
                            var shootableWeapon = itemActions[j] as MyShootableWeapon;
...
//             }
        ...
        private void HitscanFire(float strength)
        {
            ...
            var firePoint = FirePointLocation.position;//useLookPosition ? m_LookSource.LookPosition(true) : m_ShootableWeaponPerspectiveProperties.FirePointLocation.position;
            var fireDirection = FireDirection(firePoint);
            var fireRay = new Ray(firePoint, fireDirection);
            // Prevent t
            // he ray between the character and the look source from causing a false collision.
        ...
        private Vector3 FireDirection(Vector3 firePoint)
        {
            ...
            if (m_Spread > 0) {
                direction += Quaternion.AngleAxis(UnityEngine.Random.Range(0, 360), direction) * transform.up * UnityEngine.Random.Range(0, m_Spread / 360);
            }
        ...
        private void SpawnMuzzleFlash()
        {
            ...
            var muzzleFlashObj = MuzzleFlash.GetCachedComponent<MuzzleFlash>();
